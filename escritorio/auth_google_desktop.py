...
def _load_env():
    # auth_google_desktop.py está en /escritorio, la raíz es parents[1]
...
def _load_env_candidates() -> None:
    candidates = [
        Path.cwd() / ".env",
        Path(__file__).resolve().parents[1] / ".env",  # si auth_google_desktop.py está en /escritorio
...
def _get_install_dir() -> Path:
    # Cuando está compilado (exe), sys.executable apunta al exe instalado
...
def _load_google_client_id() -> str | None:
    cfg = _get_install_dir() / "desktop_config.json"
    if cfg.exists():
        try:
