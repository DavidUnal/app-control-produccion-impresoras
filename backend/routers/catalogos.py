...
def _pick_col(conn: Connection, table: str, candidates: list[str]) -> str | None:
    for c in candidates:
...
def _is_lona(conn: Connection, material_id: int) -> bool:
    ...
    row = conn.execute(
        text("""
            SELECT UPPER(TRIM(Material)) AS nombre
...
@router.delete("/materiales/{material_id}", summary="Ocultar material (soft delete)")
def borrar_material(material_id: int, force: int = Query(0, ge=0, le=1), conn: Connection = Depends(get_conn)):
    mid = int(material_id)

    row = conn.execute(
        text("SELECT id_catalogo FROM catalogo_materiales WHERE id_catalogo = :id LIMIT 1"),
        {"id": mid},
    ).first()
    if not row:
        raise HTTPException(404, detail="Material no encontrado.")

    # SOLO OCULTAR: no borramos existencias ni órdenes (ignore force)
    conn.execute(
        text("""
    ...
    conn.commit()
    return {"ok": True, "msg": "Material ocultado. Histórico se conserva."}
...
def crear_material(payload: Dict[str, Any], conn: Connection = Depends(get_conn)):
    ...
    row = conn.execute(
        text(f"""
            SELECT {id_col} AS id, deleted_at, is_active
            FROM {table}
            WHERE UPPER({name_col}) = UPPER(:n)
            LIMIT 1
        """),
        {"n": nombre},
    ).mappings().first()
    ...
    if row:
        # si estaba oculto, reactivar
        if row.get("deleted_at") is not None or int(row.get("is_active") or 1) == 0:
            conn.execute(text(f"""
                UPDATE {table}
            ...
            """), {"id": int(row["id"])})
            conn.commit()
            return {"id": int(row["id"]), "nombre": nombre, "existed": True, "reactivated": True}
...
def crear_tipo_laminado(payload: Dict[str, Any], conn: Connection = Depends(get_conn)):
    ...
    id_col = "id_laminado"
    name_col = "nombre_laminado"

    # Buscamos si existe, y también traemos su estado de eliminación
    row = conn.execute(
        text(f"""
            SELECT {id_col} AS id, deleted_at, is_active 
            FROM {table} 
    ...
    if row:
        # Si el laminado ya existía pero estaba oculto (eliminado lógicamente), lo reactivamos
        if row.get("deleted_at") is not None or int(row.get("is_active") or 1) == 0:
            conn.execute(text(f"""
                UPDATE {table}
                SET deleted_at = NULL, is_active = 1
        ...
        # Si ya existía y estaba activo, simplemente avisamos que ya existe
        return {"id": int(row["id"]), "nombre": nombre, "existed": True}

    # Si es totalmente nuevo, lo insertamos normalmente
    conn.execute(text(f"INSERT INTO {table} ({name_col}) VALUES (:n)"), {"n": nombre})
    new_id = conn.execute(text("SELECT LAST_INSERT_ID()")).scalar()
    conn.commit()
...
@router.delete("/laminados/marcas/{marca_id}")
def eliminar_marca_laminado(marca_id: int, conn: Connection = Depends(get_conn)):
    # 1. Verificamos si existe
    row = conn.execute(
        text("SELECT id_marca FROM catalogo_marcas_laminado WHERE id_marca = :id"),
        {"id": marca_id}
    ).fetchone()
    
    if not row:
        raise HTTPException(status_code=404, detail="Marca no encontrada en la base de datos")
        
    try:
        # 2. Intentamos eliminarla con SQL puro
        conn.execute(
            text("DELETE FROM catalogo_marcas_laminado WHERE id_marca = :id"),
            {"id": marca_id}
        )
        conn.commit()
        return {"ok": True, "message": "Marca eliminada correctamente"}
    except IntegrityError:
        # 3. Protección por si ya tiene existencias u órdenes
        conn.rollback()
        raise HTTPException(
            status_code=400, 
            detail="No se puede eliminar esta marca porque ya está asociada a inventario u órdenes."
        )



@router.delete("/laminados/tipos/{tipo_id}", summary="Ocultar tipo de laminado (soft delete)")
def eliminar_tipo_laminado(
    tipo_id: int,
    force: bool = Query(False, description="(Ignorado) Ya no se borran existencias, solo se oculta el tipo"),
    conn: Connection = Depends(get_conn),
):
    ...
    lam = _one(
        conn,
        """
        SELECT id_laminado AS id,
               nombre_laminado AS nombre,
               deleted_at,
               is_active
        FROM catalogo_tipos_laminado
        WHERE id_laminado = :tid
        LIMIT 1
        """,
        {"tid": int(tipo_id)},
    )
    if not lam:
        raise HTTPException(status_code=404, detail="Tipo de laminado no encontrado.")

    # 2) Protege "SIN LAMINAR"
    nombre = (lam.get("nombre") or "").strip().upper()
    if nombre in {"SIN LAMINAR", "SIN LAMINADO"}:
        raise HTTPException(status_code=409, detail="No se puede eliminar/ocultar el tipo 'SIN LAMINAR'.")

    # 3) Si ya está oculto, devolvemos OK idempotente
    if lam.get("deleted_at") is not None or int(lam.get("is_active") or 1) == 0:
        return {"ok": True, "msg": "Ya estaba oculto.", "lam_tipo_id": int(tipo_id)}

    # 4) Soft delete (NO borrar existencias, NO borrar órdenes)
    conn.execute(
        text("""
            UPDATE catalogo_tipos_laminado
            SET deleted_at = NOW(),
                is_active = 0
            WHERE id_laminado = :tid
            LIMIT 1
    ...
    return {"ok": True, "msg": "Tipo de laminado ocultado.", "lam_tipo_id": int(tipo_id)}
...
def crear_marca_laminado(payload: dict = Body(...), conn: Connection = Depends(get_conn)):
    ...
    if not nombre:
        raise HTTPException(400, detail="El nombre de la marca es obligatorio.")
    ...
    conn.execute(text(f"""
        ...
        VALUES (:n)
        ON DUPLICATE KEY UPDATE {id_col} = LAST_INSERT_ID({id_col})
    """), {"n": nombre})

    new_id = conn.execute(text("SELECT LAST_INSERT_ID()")).scalar()
    if not new_id:
        new_id = conn.execute(
            text(f"SELECT {id_col} FROM {table} WHERE {name_col}=:n"),
            {"n": nombre}
        ).scalar()
