...
import requests
from PySide6.QtCore import Qt, QDate, QSize, QUrl, QTimer, Signal, QObject , QSettings
from PySide6.QtWidgets import (
    ...
    QSizePolicy, QInputDialog, QGraphicsDropShadowEffect, QSpacerItem, QToolButton, QMenu ,
    QStackedLayout
)
from PySide6.QtGui import (
    QDesktopServices, QShortcut, QKeySequence, QIcon, QPixmap, QFont, QAction, QColor, QPainter, 
)
...
class PagAltasCatalogo(QWidget):
    ...
    def _build(self):
        ...
        btn_del_item = QPushButton("Eliminar (Material/Laminado)")
        ...
        fila_btns.addWidget(btn_del_item)
    ...
    def _find_laminado_tipo_by_name(self, nombre_buscar: str):
        items = api_get("/catalogos/laminados/tipos", self) or []
    ...
    def _delete_item(self):
        """
        Elimina según el catálogo seleccionado.
        - Material -> usa tu delete actual
        - Laminado -> delete de tipo de laminado
        """
        if self.cmb_tipo.currentText() == "Material":
            self._delete_material()
        else:
            self._delete_laminado()




    def _api_delete_try(self, paths: list[str], params: dict | None = None):
        """
        Intenta varias rutas DELETE hasta que una funcione.
        Útil cuando el backend puede tener el DELETE con paths distintos.
        """
        last_ok = None
        for p in paths:
            try:
                last_ok = api_delete(p, self, params=params)
                if last_ok:
                    return last_ok
            except Exception:
                # api_delete ya suele mostrar error; igual seguimos intentando otras rutas
                pass
        return last_ok


    def _delete_laminado(self):
        # Valida
        nombre = self.ed_nombre.text().strip()
        if not nombre:
            QMessageBox.warning(self, "Falta nombre", "Escribe el nombre del laminado (TIPO) a eliminar.")
            return

        lam = self._find_laminado_tipo_by_name(nombre)
        if not lam:
            QMessageBox.warning(self, "No encontrado", f"No existe un laminado (tipo) llamado:\n{nombre}")
            return

        lam_id = int(lam["id"])
        if QMessageBox.question(
            self, "Confirmar",
            f"¿Eliminar el laminado '{lam.get('nombre')}' (ID {lam_id})?\n"
            "Nota: esto elimina el TIPO de laminado. (Marcas/Existencias pueden depender del backend)",
            QMessageBox.Yes | QMessageBox.No
        ) != QMessageBox.Yes:
            return

        # 👇 probamos rutas comunes (ajusta si tu backend usa otra)
        ok = self._api_delete_try([
            f"/catalogos/laminados/tipos/{lam_id}",        # REST común
            f"/catalogos/laminados/tipos/delete/{lam_id}", # alternativa común
        ])

        if ok:
            QMessageBox.information(self, "OK", "Laminado eliminado.")
            self.lbl_status.setText("Laminado eliminado ✔")
        else:
            QMessageBox.critical(self, "Error", "No se pudo eliminar el laminado (revisa endpoint DELETE en backend).")
    ...
    def _find_laminado_by_name(self, nombre_buscar: str):
        items = api_get("/catalogos/laminados/tipos", self) or []
        nb = (nombre_buscar or "").strip().upper()
        for r in items:
            if (r.get("nombre", "").strip().upper() == nb):
    ...
    def _delete_item(self):
        ...
        nombre = self.ed_nombre.text().strip()
        if not nombre:
            QMessageBox.warning(self, "Falta nombre", "Escribe el nombre a eliminar.")
            return

        # --- MATERIAL ---
        if tipo == "Material":
            nombre_norm = self._normalized_nombre_material(nombre, self.cmb_subcat.currentText())
            mat = self._find_material_by_name(nombre_norm)
            if not mat:
            ...
            if QMessageBox.question(
                self, "Confirmar",
                f"¿Eliminar el material '{nombre_norm}' (ID {mat['id']})?"
            ...
            if r:
                ...
                # si ya tienes la señal:
                if hasattr(self, "catalog_changed"):
                    self.catalog_changed.emit("Material")
            return

        # --- LAMINADO ---
        lam = self._find_laminado_by_name(nombre)
        if not lam:
            QMessageBox.warning(self, "No encontrado", f"No existe un laminado (tipo) llamado:\n{nombre}")
            return
        ...
        if QMessageBox.question(
            self, "Confirmar",
            f"¿Eliminar el laminado '{nombre}' (ID {lam['id']})?\n"
            "Nota: esto elimina el TIPO de laminado. (Marcas/Existencias pueden depender del backend)",
            QMessageBox.Yes | QMessageBox.No
        ) != QMessageBox.Yes:
            return

        # Endpoint sugerido (ajústalo si tu backend usa otro)
        r = api_delete(f"/catalogos/laminados/tipos/{int(lam['id'])}", self)
        if r:
            QMessageBox.information(self, "OK", "Laminado eliminado.")
            self.lbl_status.setText("Laminado eliminado ✔")
            if hasattr(self, "catalog_changed"):
                self.catalog_changed.emit("Laminado")
    ...
    def _delete_medidas(self):
        ...
        if ok:
            ...
            self.catalog_changed.emit(self.cmb_tipo.currentText())  
