# desktop/app.py
import sys, json, requests, os, atexit, datetime
from functools import partial
from pathlib import Path

from PySide6.QtCore import Qt, QDate, QSize, QUrl, QTimer, Signal, QObject
from PySide6.QtWidgets import (
    ...
    QPlainTextEdit, QDialog, QFrame, QAbstractItemView, QStyle, QListWidgetItem, QSizePolicy,
    QGraphicsDropShadowEffect, QSpacerItem, QToolButton, QStackedLayout
)
from PySide6.QtGui import QDesktopServices, QShortcut, QKeySequence, QIcon, QPixmap, QFont, QAction
from PySide6.QtWebSockets import QWebSocket

# Tema (si tu módulo está en el mismo paquete)
try:
    from .ui_theme import apply_theme
except ImportError:
    from ui_theme import apply_theme

# stdout line-buffered (Windows suele agradecerlo en terminales)
try:
    sys.stdout.reconfigure(line_buffering=True)
except Exception:
    pass

# --- Imports condicionales (según ejecutes como paquete o script) ---
try:
    from .admin_solicitudes import AdminSolicitudesFrame  # paquete
except ImportError:
    from admin_solicitudes import AdminSolicitudesFrame   # script
...
try:
    ...
    try:
        ...
        signin_with_google = None   # desactiva el botón si no está disponible

# ⚠️ NO importamos un api_post “externo” para no confundir con el wrapper local
# (más abajo definimos api_get/api_post/api_delete que usa todo tu código).

# --- Rutas/constantes ---
ROOT_DIR = Path(__file__).resolve().parents[1]           # carpeta “Python Ficheros”
BG_LOGIN_PATH: Path = ROOT_DIR / "fondo de imagen.jpg"   
BG_APP_PATH:   Path = ROOT_DIR / "fondo_app.jpg"        # nuevo fondo 2960x1664
APP_BG_PATH:  Path = BG_APP_PATH              # tu imagen .jpg


# LOGO: se intenta detectar automáticamente varias extensiones



# --- Rutas/constantes (compatibles con PyInstaller) ---
def _resource_root() -> Path:
    # PyInstaller: recursos dentro de _MEIPASS
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        return Path(sys._MEIPASS)
    # Modo normal (python)
    return Path(__file__).resolve().parents[1]  # ajusta si tu estructura es distinta
...
# LOGO
LOGO_PATH: Path = ROOT_DIR / "logo marca.ico"


def _find_logo_in_repo() -> Path | None:
    root = ROOT_DIR
    candidates = [
...
LOGO_PATH = ROOT_DIR / "logo marca.ico"
...
def resource_path(*parts: str) -> str:
    ...
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        ...
        # asume que app.py está en /escritorio/app.py y el ROOT_DIR es su padre
...
def get_pendientes(base_url: str, token: str):
    r = requests.get(f"{base_url}/auth/admin/pendientes",
                     headers=_auth_headers(token), timeout=10)
    r.raise_for_status()
    return r.json()

def aprobar_usuario(base_url: str, token: str, uid: int, rol: str):
    r = requests.post(f"{base_url}/auth/admin/usuarios/{uid}/aprobar",
                      json={"rol": rol}, headers=_auth_headers(token), timeout=10)
    r.raise_for_status()
    return r.json()

def set_rol(base_url: str, token: str, uid: int, rol: str):
    r = requests.post(f"{base_url}/auth/admin/usuarios/{uid}/set-rol",
                      json={"rol": rol}, headers=_auth_headers(token), timeout=10)
    r.raise_for_status()
    return r.json()
...
class AdminSolicitudesPage(QWidget):
    """
    Lista usuarios PENDIENTES y permite:
      - Aprobar / Cambiar rol a: ADMIN / DISENO / IMPRESION
      - Devolver a PENDIENTE (Quitar rol)
      - Refrescar
    """
    ...
    def _build(self):
        ...
        top.addWidget(QLabel("Rol:")); top.addWidget(self.cmb_rol)
        top.addStretch(1)
        ...
        self.tbl.setHorizontalHeaderLabels(["ID", "Email", "Rol", "Verificado"])
        hdr = self.tbl.horizontalHeader()
        hdr.setSectionResizeMode(QHeaderView.Interactive)
        self.tbl.setColumnWidth(0, 70)   # ID
        self.tbl.setColumnWidth(1, 280)  # Email
        self.tbl.setColumnWidth(2, 120)  # Rol
        self.tbl.setColumnWidth(3, 100)  # Verificado
    ...
    def _load(self):
        try:
            rows = get_pendientes(API, AUTH["token"]) or []
        except Exception as e:
        ...
        for r in rows:
            ...
            def set_text(c, v):
                self.tbl.setItem(i, c, QTableWidgetItem("" if v is None else str(v)))

            set_text(0, r.get("id_usuario") or r.get("id") or "")
            set_text(1, r.get("nombre_usuario") or r.get("email") or "")
            set_text(2, r.get("rol") or "")
            verif = r.get("email_verificado") or r.get("EMAIL_VERIFICADO") or 0
            set_text(3, "Sí" if int(verif) == 1 else "No")

    def _aprobar(self):
        uid = self._sel_uid()
        if not uid:
            QMessageBox.information(self, "Administración", "Selecciona una fila primero.")
            return
        rol = self.cmb_rol.currentData()
        try:
            aprobar_usuario(API, AUTH["token"], int(uid), str(rol))
            QMessageBox.information(self, "OK", f"Usuario #{uid} → {rol}")
            self._load()
        except Exception as e:
            _show_err(self, e)
    ...
    def _poner_pendiente(self):
        ...
        if not uid:
            ...
            return
        try:
            set_rol(API, AUTH["token"], int(uid), "PENDIENTE")
            QMessageBox.information(self, "OK", f"Usuario #{uid} → PENDIENTE")
            self._load()
        except Exception as e:
            _show_err(self, e)
...
class DlgLogin(QDialog):
    """
    Login estilo 'card + hero image'.
    - Izquierda: tarjeta con logo/título, usuario/contraseña y botones.
    - Derecha : imagen de fondo (fondo de imagen.jpg).
    Al aceptar, deja en self.result_data la respuesta del backend.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        ...
        # Construir panel izquierdo y derecho
        left  = self._build_left_card()
        right = self._build_right_hero()
        root.addWidget(left, 0)
        root.addWidget(right, 10)

        # Estilos (después de crear los widgets)
        self._apply_styles()

        # Accesibilidad: Enter hace login
        ...
        # Mostrar/ocultar password con botón ojo
    ...
    def _build_left_card(self) -> QWidget:
        ...
        self.ed_user.setPlaceholderText("email / usuario")
    ...
    def _build_right_hero(self) -> QWidget:
        ...
        # contenedor tipo "stack": fondo + logo superpuesto al centro
        ...
        if BG_LOGIN_PATH.exists():
            pm = QPixmap(str(BG_LOGIN_PATH))
            self.hero.setPixmap(pm)
        ...
        logo_pix = QPixmap(str(LOGO_PATH)).scaled(
            260, 260, Qt.KeepAspectRatio, Qt.SmoothTransformation  # tamaño recomendado
        )
    ...
    def _update_hero_pixmap(self):
        if not getattr(self, "_hero_pm", None):
            return
        size = self.hero.size()
        if size.width() <= 0 or size.height() <= 0:
            return

        # “Cover”: llena todo el área, aunque recorte un poco
        scaled = self._hero_pm.scaled(
            size,
            Qt.KeepAspectRatioByExpanding,    # ⬅️ clave
    ...
    def resizeEvent(self, ev):
        ...
        self._update_hero_pixmap()



    def paintEvent(self, event):
        """Dibuja fondo_app.jpg como fondo de TODO el login."""
        from PySide6.QtGui import QPainter

        # primero deja que Qt pinte normalmente (bordes, etc.)
        ...
        # Escalamos la imagen con "cover": llenar todo y recortar lo que sobre
        pm_scaled = self._bg_pm.scaled(
            target.size(),
            Qt.KeepAspectRatioByExpanding,   # llena todo, aunque recorte
            Qt.SmoothTransformation,
        )

        # Centramos el recorte
        x = (pm_scaled.width()  - target.width())  // 2
        y = (pm_scaled.height() - target.height()) // 2

        # Dibujar recorte centrado sobre la ventana
        painter.drawPixmap(
            target,
            pm_scaled,
            pm_scaled.rect().adjusted(x, y, -x, -y)
        )
        painter.end()
    ...
    def _apply_styles(self):
        ...
        QPushButton:default, QPushButton:enabled:pressed { outline: none; }
        QFrame#Hero { border-radius: 12px; }
        QFrame#Line, QFrame[objectName="Line"] {
            background: #e5e7eb; height: 1px;
        }
         QLabel#Hero {
    ...
    def _on_login(self):
        ...
        try:
            ...
            r = api_post("/auth/login", {"usuario": user, "password": pwd}, self)
            if not r or "access_token" not in r:
                raise RuntimeError("Respuesta inválida del servidor.")
            self.result_data = r
            self.accept()
        except Exception as ex:
            QMessageBox.critical(self, "Login", f"No fue posible iniciar sesión.\n{ex}")
    ...
    def _on_google_clicked(self):
        ...
        try:
            # usa el helper compatible
            data = _signin_with_google_compat(self)

            if data and (data.get("access_token") or data.get("token")):
                self.result_data = data
                self.accept()
            else:
                QMessageBox.warning(self, "Google", "No se recibió token al autenticar con Google.")
        except Exception as ex:
            QMessageBox.critical(self, "Google", f"Fallo al autenticarse con Google.\n{ex}")

    def _on_register(self):
        # aquí puedes abrir tu página de registro si la tienes
        QMessageBox.information(self, "Registro", "Contacta al administrador para crear tu usuario.")
...
class DlgRegister(QDialog):
    """
    Registro por email:
      1) Enviar código a correo (POST /auth/register/request-code)
      2) Confirmar código (POST /auth/register/confirm)
         - contraseña (opcional) con verificación
         - rol_solicitado: ADMIN / DISENO / IMPRESION
      3) Queda 'PENDIENTE' hasta aprobación ADMIN
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        ...
        # Campos
        self.ed_email = QLineEdit(); self.ed_email.setPlaceholderText("tu_usuario@gmail.com")

        self.ed_pass  = QLineEdit(); self.ed_pass.setEchoMode(QLineEdit.Password)
        self.ed_pass.setPlaceholderText("Contraseña (opcional)")
        self.ed_pwd2  = QLineEdit(); self.ed_pwd2.setEchoMode(QLineEdit.Password)
        self.ed_pwd2.setPlaceholderText("Repite la contraseña")

        self.cmb_rol = QComboBox()
        self.cmb_rol.addItems(["Administrativo", "Diseño", "Impresión"])

        self.ed_code  = QLineEdit(); self.ed_code.setPlaceholderText("Código de 6 dígitos")

        # Botones
        self.btn_send = QPushButton("Enviar código")
        self.btn_conf = QPushButton("Confirmar y solicitar acceso")
        self.btn_cancel = QPushButton("Cerrar")

        self.btn_send.clicked.connect(self._send_code)
        self.btn_conf.clicked.connect(self._confirm)
        self.btn_cancel.clicked.connect(self.reject)

        # Layout
        form.addRow("Correo Google", self.ed_email)
        form.addRow("Contraseña", self.ed_pass)
        form.addRow("Repite contraseña", self.ed_pwd2)
        form.addRow("Rol solicitado", self.cmb_rol)
        form.addRow(self.btn_send)
        form.addRow("Código recibido", self.ed_code)

        row = QHBoxLayout()
        row.addWidget(self.btn_conf)
    ...
    def _send_code(self):
        ...
        if not email or "@" not in email:
            QMessageBox.warning(self, "Registro", "Escribe un correo válido.")
            return
        r = api_post("/auth/register/request-code", {"email": email}, self)
        if r:
            # En prod no mostramos dev_code; si el backend lo devuelve, sólo informamos “correo enviado”
            QMessageBox.information(self, "Registro", "Te enviamos un código. Revisa tu correo.")
    ...
    def _confirm(self):
        ...
        if not code or len(code) != 6 or not code.isdigit():
            QMessageBox.warning(self, "Registro", "Código inválido (debe tener 6 dígitos).")
            return

        # Si el usuario decidió poner contraseña, validar longitud y coincidencia
        if p1 or p2:
            if len(p1) < 8:
        ...
        r = api_post("/auth/register/confirm", payload, self)
        if r:
            QMessageBox.information(
                self, "Registro",
                "Solicitud enviada. Tu cuenta quedó PENDIENTE hasta aprobación del administrador."
            )
            self.accept()
...
class PagDisenio(QWidget):
    ...
    def _pick_ruta(self):
        ...
        # Preguntar si quiere seleccionar archivo o carpeta
    ...
    def _on_material_change(self):
        ...
        # Marcas del material (adhesivo filtra 10/12 OZ/PUBLIMASTER; lona muestra 10/12/PUBLIMASTER)
...
class PagInvMaterial(_InvBase):
    ...
    def _build(self):
        ...
        self.sb_delta = QDoubleSpinBox(); self.sb_delta.setDecimals(2); self.sb_delta.setMinimum(-10_000_000); self.sb_delta.setMaximum(10_000_000)
        ...
        self.tbl_exist = QTableWidget(0, 4)
        ...
        for i, w in enumerate([260, 160, 110, 130]):
    ...
    def _on_mat(self):
        if self.cmb_mat.currentIndex() < 0: return
    ...
    # ---------- tabla: helpers ----------
    def _add_exist_row(self, material_txt: str, marca_txt: str, ancho_cm: float, stock_cm: float):
    ...
    def _parse_selected_exist_material(self):
        """Devuelve {'marca_id': int|None, 'ancho_cm': float} desde la fila seleccionada de la tabla."""
        ...
        if not marca_txt or marca_txt.lower().startswith("(sin marca"):
            ...
            for i in range(self.cmb_marca.count()):
                if (self.cmb_marca.itemText(i) or "").strip() == marca_txt:
                    mk = int(self.cmb_marca.itemData(i)); break
    ...
    def _buscar(self):
        ...
        for a in anchos:
            if a is None:
                continue
            for m in marcas:
                qs = f"/inventario/existencias?material_id={mid}&ancho_cm={a}"
                if m is not None:
                    qs += f"&marca_id={int(m)}"
                rows = api_get(qs, self) or []
                for r in rows:
                    self._add_exist_row(
                        mat_txt,
                        _marca_txt(r.get("marca_id")),
                        float(r.get("ancho_cm", 0.0)),
                        float(r.get("stock_cm", 0.0)),
                    )
                    added += 1
    ...
    def _ensure(self):
        ...
        if not mid or an is None:
            ...
            return
        payload = {"material_id": mid, "marca_id": (mk if self.cmb_marca.currentIndex()>=0 else None), "ancho_cm": an}
        if api_post("/inventario/ensure", payload, self):
    ...
    def _ajuste(self, sgn: int):
        ...
        if delta == 0:
            ...
            return
        payload = {"material_id": mid, "marca_id": (mk if self.cmb_marca.currentIndex()>=0 else None),
                   "ancho_cm": an, "delta_cm": delta, "obs": self.ed_obs.text().strip() or None, "usuario": None}
        r = api_post("/inventario/ajuste", payload, self)
        if r:
    ...
    def _delete(self):
        ...
        params = {"material_id": mid, "ancho_cm": an}
        if mk is not None:
            params["marca_id"] = mk

        r = api_delete("/inventario/delete", self, params)
        if r:
    ...
    def _movs(self):
        ...
        if not mid or an is None:
            QMessageBox.warning(self, "Aviso", "Selecciona material y ancho.")
            return
        qs = f"/inventario/movimientos?material_id={mid}&ancho_cm={an}&limit=20"
        if self.cmb_marca.currentIndex()>=0: qs += f"&marca_id={mk}"
        rows = api_get(qs, self) or []
        if not rows:
            QMessageBox.information(self, "Movimientos", "No hay movimientos recientes.")
            return
        msg = "\n".join([f"{r['fecha']}  {r['tipo']:<9}  {r.get('cantidad_cm',0)} cm | Ord:{r.get('id_orden','-')} [{r.get('observaciones','')}]" for r in rows])
...
class PagInvLaminado(_InvBase):
    ...
    def _build(self):
        ...
        self.tbl_exist.setHorizontalHeaderItem(3, it3)
        self._beautify_table(self.tbl_exist)
        for i, w in enumerate([220, 160, 110, 130]):
            self.tbl_exist.setColumnWidth(i, w)
    ...
    # ---------- tabla: helpers ----------
    def _add_exist_row(self, tipo_txt: str, marca_txt: str, ancho_cm: float, stock_cm: float):
        r = self.tbl_exist.rowCount()
        self.tbl_exist.insertRow(r)
        def put(c, text, align_right=False):
            it = QTableWidgetItem("" if text is None else str(text))
    ...
    def _parse_selected_exist_lam(self):
        """Devuelve {'lam_marca_id': int|None, 'ancho_cm': float} desde la fila seleccionada."""
        r = self.tbl_exist.currentRow()
        if r < 0:
            return None
        marca_txt = (self.tbl_exist.item(r, 1).text() if self.tbl_exist.item(r,1) else "").strip()
    ...
    def _buscar(self):
        ...
        for an in anchos:
            if an is None:
                continue
            for mk in marcas:
                qs = f"/inventario/laminados/existencias?lam_tipo_id={t}&ancho_cm={an}"
                if mk is not None:
                    qs += f"&lam_marca_id={mk}"
                rows = api_get(qs, self) or []
                for r in rows:
                    tipo_txt  = tipo_map.get(r.get("lam_tipo_id"), "LAMINADO")
                    marca_txt = marca_map.get(r.get("lam_marca_id"), "(sin marca)")
                    self._add_exist_row(
                        tipo_txt,
                        marca_txt,
                        float(r.get("ancho_cm", 0.0)),
    ...
    def _ensure(self):
        ...
        if not t or a is None:
            ...
            return
        payload = {"lam_tipo_id": t, "lam_marca_id": (m if self.cmb_marca.currentIndex()>=0 else None), "ancho_cm": a}
        if api_post("/inventario/laminados/ensure", payload, self):
            QMessageBox.information(self, "OK", "Combinación asegurada (stock=0).")
            self._buscar()
    ...
    def _ajuste(self, sgn: int):
        ...
        if delta == 0:
            QMessageBox.warning(self, "Aviso", "Δ Ajuste debe ser distinto de 0.")
            return
        payload = {"lam_tipo_id": t, "lam_marca_id": (m if self.cmb_marca.currentIndex()>=0 else None),
                   "ancho_cm": a, "delta_cm": delta, "obs": self.ed_obs.text().strip() or None, "usuario": None}
        r = api_post("/inventario/laminados/ajuste", payload, self)
        if r:
    ...
    def _delete(self):
        ...
        params = {"lam_tipo_id": t, "ancho_cm": a}
        if m is not None:
            params["lam_marca_id"] = m

        r = api_delete("/inventario/laminados/delete", self, params)
        if r:
            QMessageBox.information(self, "OK", "Combinación eliminada.")
            self._buscar()
    ...
    def _movs(self):
        ...
        if not t or a is None:
            QMessageBox.warning(self, "Aviso", "Selecciona tipo y ancho.")
            return
        qs = f"/inventario/laminados/movimientos?lam_tipo_id={t}&ancho_cm={a}&limit=20"
        if self.cmb_marca.currentIndex()>=0: qs += f"&lam_marca_id={m}"
        rows = api_get(qs, self) or []
        if not rows:
            QMessageBox.information(self, "Movimientos", "No hay movimientos recientes.")
            return
...
class PagAdmin(QWidget):
    def __init__(self, base_url: str, token: str, parent=None):
        super().__init__(parent)
        lay = QVBoxLayout(self)
        self.panel = AdminSolicitudesFrame(self, base_url, token)
...
class PagAltasCatalogo(QWidget):
    ...
    def _build(self):
        ...
        # subcat solo aplica para Material
        self.cmb_tipo.currentTextChanged.connect(
            lambda _: self.cmb_subcat.setEnabled(self.cmb_tipo.currentText()=="Material")
        )

        self.ed_nombre = QLineEdit()
        self.ed_nombre.setPlaceholderText("Nombre (p.ej. 'LONA REVERSO BLANCO' o 'ADH ALTA ADHERENCIA')")

        self.txt_marcas = QPlainTextEdit()
        self.txt_marcas.setPlaceholderText("Marcas (una por línea). Opcional.")
        self.txt_marcas.setFixedHeight(80)

        self.ed_medidas = QLineEdit()
        self.ed_medidas.setPlaceholderText("Medidas separadas por coma (ej: 71, 81, 105, 122, 127)")

        self.chk_seed = QCheckBox("Sembrar existencias (stock=0) para todas las marcas × medidas")
        # NUEVO: casilla para forzar borrado de material
        self.chk_del_force = QCheckBox("Forzar eliminación del material (borra existencias)")

        # Botones
        btn_guardar   = QPushButton("Guardar / Crear")
        btn_del_mat   = QPushButton("Eliminar material")
        btn_del_meds  = QPushButton("Eliminar medidas")

        btn_guardar.clicked.connect(self._save)
        btn_del_mat.clicked.connect(self._delete_material)
        btn_del_meds.clicked.connect(self._delete_medidas)

        form.addRow("Catálogo", self.cmb_tipo)
        form.addRow("Subcategoría", self.cmb_subcat)
        form.addRow("Nombre", self.ed_nombre)
        form.addRow("Marcas", self.txt_marcas)
        form.addRow("Medidas (cm)", self.ed_medidas)
        form.addRow(self.chk_seed)
        form.addRow(self.chk_del_force)

        fila_btns = QHBoxLayout()
        fila_btns.addWidget(btn_guardar)
        fila_btns.addWidget(btn_del_mat)
        fila_btns.addWidget(btn_del_meds)
        fila_btns.addStretch(1)
        form.addRow(fila_btns)
        ...
        # estado inicial
        self.cmb_subcat.setEnabled(True)

    # ---------------- utils ----------------
    def _parselist_floats(self, text_):
        out = []
        for token in (text_ or "").replace(";", ",").split(","):
        ...
        return out
        # ---------- utils de borrado ----------
    def _normalized_nombre_material(self, nombre: str, subcat: str) -> str:
        # Igual que cuando creas: si es Lona y no empieza por 'LONA ', lo anteponemos
        if subcat == "Lona" and not (nombre or "").upper().startswith("LONA"):
            return f"LONA {nombre}".strip()
        return nombre

    def _find_material_by_name(self, nombre_buscar: str):
        # Busca por igualdad case-insensitive
        items = api_get("/catalogos/materiales", self) or []
        nb = (nombre_buscar or "").strip().upper()
        for r in items:
            if (r.get("nombre","").strip().upper() == nb):
                return r
        return None

    # ---------- acciones ----------
    def _delete_material(self):
        if self.cmb_tipo.currentText() != "Material":
            QMessageBox.information(self, "Eliminar", "Cambia 'Catálogo' a Material para eliminar.")
            return

        nombre = self.ed_nombre.text().strip()
        if not nombre:
            QMessageBox.warning(self, "Falta nombre", "Escribe el nombre del material a eliminar.")
            return

        # Normaliza igual que al crear
        nombre = self._normalized_nombre_material(nombre, self.cmb_subcat.currentText())

        mat = self._find_material_by_name(nombre)
        if not mat:
            QMessageBox.warning(self, "No encontrado", f"No existe un material llamado:\n{nombre}")
            return

        force = 1 if self.chk_del_force.isChecked() else 0
        if QMessageBox.question(
            self, "Confirmar",
            f"¿Eliminar el material '{nombre}' (ID {mat['id']})?"
            + ("\nTambién se borrarán existencias." if force else ""),
            QMessageBox.Yes | QMessageBox.No
        ) != QMessageBox.Yes:
            return

        r = api_delete(f"/catalogos/materiales/{int(mat['id'])}", self, params={"force": force})
        if r:
            QMessageBox.information(self, "OK", "Material eliminado.")
            self.lbl_status.setText("Material eliminado ✔")

    def _delete_medidas(self):
        # Borra medidas del catálogo (adhesivo o lona)
        medidas = self._parselist_floats(self.ed_medidas.text())
        if not medidas:
            QMessageBox.warning(self, "Faltan medidas", "Escribe al menos una medida a borrar.")
            return

        # ¿De qué catálogo borramos?
        if self.cmb_tipo.currentText() == "Material":
            categoria = "LONA" if self.cmb_subcat.currentText() == "Lona" else "ADHESIVO"
        else:
            # En Laminado tus medidas usan el catálogo de adhesivos
            categoria = "ADHESIVO"

        ok, err = 0, 0
        for w in sorted(set(medidas)):
            res = api_delete("/catalogos/medidas", self, params={"categoria": categoria, "ancho_cm": float(w)})
            if res:
                ok += 1
            else:
                err += 1
        ...
        QMessageBox.information(
            self, "Eliminar medidas",
            f"Categoría: {categoria}\nEliminadas: {ok}\nErrores: {err}"
        )
        if ok:
            self.lbl_status.setText("Medidas eliminadas ✔")


    def _parselist_str(self, text_):
        lines = []
        for ln in (text_ or "").splitlines():
            ln = ln.strip()
            if ln:
                lines.append(ln)
        return lines
    ...
    def _save(self):
        ...
        nombre = self.ed_nombre.text().strip()
        if not nombre:
            QMessageBox.warning(self, "Falta nombre", "Ingresa el nombre.")
            return

        # medidas y marcas
        medidas = self._parselist_floats(self.ed_medidas.text())
        marcas = self._parselist_str(self.txt_marcas.toPlainText())
        marcas  = list(dict.fromkeys(marcas))            # quita duplicados preservando orden
        medidas = sorted(set(medidas))                   # quita duplicados y ordena


        try:
            if tipo == "Material":
                # Prefijo opcional para clasificar como LONA (tu backend lo detecta por nombre)
                if subcat == "Lona" and not nombre.upper().startswith("LONA"):
                    nombre = f"LONA {nombre}".strip()
                ...
                marca_ids = []
                for m in marcas:
                    rr = api_post("/catalogos/marcas/create", {"nombre": m}, self)
                    if rr:
                        marca_ids.append(int(rr["id"]))
                if not marca_ids:
                    marca_ids = [None]  # sin marca

                # Añadir medidas en el catálogo correspondiente
                categoria = "LONA" if subcat == "Lona" else "ADHESIVO"
                for w in medidas:
                    api_post("/catalogos/medidas/add", {"categoria": categoria, "ancho_cm": w}, self)

                # Sembrar existencias (stock=0)
                if self.chk_seed.isChecked():
                    for w in medidas:
                        for mk in marca_ids:
                            payload = {"material_id": mat_id, "ancho_cm": float(w), "marca_id": mk}
                            api_post("/inventario/ensure", payload, self)

                QMessageBox.information(self, "OK", f"Material creado (ID {mat_id}).")

            else:  # Laminado
                r = api_post("/catalogos/laminados/tipos/create", {"nombre": nombre}, self)
                if not r:
                    return
                lam_tipo_id = int(r["id"])

                lam_marca_ids = []
                for m in marcas:
                    rr = api_post("/catalogos/laminados/marcas/create", {"nombre": m}, self)
                    if rr:
                        lam_marca_ids.append(int(rr["id"]))
                if not lam_marca_ids:
                    lam_marca_ids = [None]

                # Las medidas de laminado usan las de adhesivo en tu backend
                for w in medidas:
                    api_post("/catalogos/medidas/add", {"categoria": "ADHESIVO", "ancho_cm": w}, self)

                if self.chk_seed.isChecked():
                    for w in medidas:
                        for mk in lam_marca_ids:
                            payload = {"lam_tipo_id": lam_tipo_id, "ancho_cm": float(w), "lam_marca_id": mk}
                            api_post("/inventario/laminados/ensure", payload, self)

                QMessageBox.information(self, "OK", f"Laminado creado (ID {lam_tipo_id}).")
...
class PagInventario(QWidget):
    def __init__(self):
        ...
        tabs.addTab(PagInvMaterial(), "Materiales")
        tabs.addTab(PagInvLaminado(), "Laminados")
        tabs.addTab(PagAltasCatalogo(), "Altas")
        lay = QVBoxLayout(self)
        lay.addWidget(tabs)
...
class PagReportes(QWidget):
    ...
    "Merma cancel(cm)","Ruta","Abrir","Guardar"
    ]
    ...
    def _build(self):
        ...
        self.dt_desde.setDate(today.addMonths(-1))
        self.dt_desde.setDisplayFormat("yyyy-MM-dd")
        self.dt_hasta.setDisplayFormat("yyyy-MM-dd")

        self.ed_consec = QLineEdit(); self.ed_consec.setPlaceholderText("Buscar consecutivo…")
        ...
        top.addWidget(btn_ajustar);  top.addWidget(btn_refrescar)

        lay.addWidget(filter_bar)  # correcto
        ...
        self.tbl.setAlternatingRowColors(True)
        self.tbl.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.tbl.setSelectionMode(QAbstractItemView.SingleSelection)  # <-- FIX
        self.tbl.verticalHeader().setVisible(False)
        self.tbl.verticalHeader().setDefaultSectionSize(36)
        ...
        # --- Anchos sugeridos ---
        widths = [60,110,140,110,110,120,200,95,90,60,110,90,90,110,110,105,115,130,380,70,110]
    ...
    def _paint_row_bg(self, row: int, bg_hex: str):
        ...
        col = QColor(bg_hex)
        # pinta todas las celdas "item"
        for c in range(self.tbl.columnCount()):
            it = self.tbl.item(row, c)
            if it:
                it.setBackground(col)
            # si hay widgets, sólo damos fondo a los que tiene sentido (p.ej. el QLineEdit de Consec)
            w = self.tbl.cellWidget(row, c)
            # columna 5 = Consec (QLineEdit) en tu tabla de reportes
            if c == 5 and w is not None:
                w.setStyleSheet((w.styleSheet() or "") + f"; background:{bg_hex};")


        # --- util de números seguros ---
    ...
    # Regla: (impreso_cm * rep) + desp_largo_cm + (espacio_reps_cm * rep)
    # 'espacio_reps_cm' puede no venir en estos endpoints -> asumimos 0.
    def _calc_consumo_esperado(self, r: dict) -> float:
        ...
        return max(0.0, imp * max(rep, 0) + desp_l + esp * max(rep, 0))




    # -------- Fetch helpers (usa los endpoints existentes) --------
    def _fetch_finalizadas(self):

        # 1er intento: endpoint específico
        rows = api_get("/ordenes/finalizadas?limite=1000", self)
        # Fallback si falló (None), o si vino lista vacía por error
        if not rows:
        ...
        for r in rows or []:
            rr = {
                ...
                "lam_consumo_cm": r.get("lam_consumo_cm") or r.get("LAM_CONSUMO_CM"),
                "merma_cancel_cm": r.get("merma_cancel_cm") or r.get("MERMA_CANCEL_CM"),
                # si algún backend ya enviara 'espacio_reps_cm', se usará en el cálculo
                "espacio_reps_cm": r.get("espacio_reps_cm") or 0,
            }
            # Calculamos aquí para no depender de que exista en origen
    ...
    def _load(self):
        ...
        # 2) Filtros de fecha y consecutivo (cliente)
        ...
        # Guardar lo que el usuario ve (para exportar)
    ...
    def _add_row(self, r: dict):
        ...
        set_text(2, r.get("consumo_esp_cm",""))   # col 2
        ...
        # 18: Ruta
        set_text(18, r.get("ruta",""))

        # 19: Abrir
        btn_open = QPushButton("Abrir")
        ruta_txt = r.get("ruta","")
        btn_open.clicked.connect(lambda _=False, p=ruta_txt: self._abrir_ruta(p))
        self.tbl.setCellWidget(i, 19, btn_open)

        # 20: Guardar (consecutivo)
        btn = QPushButton("Guardar")
        btn.clicked.connect(partial(self._save_consec, i, int(r.get("id_orden",0) or 0)))
        self.tbl.setCellWidget(i, 20, btn)

        # Pintar AHORA (cuando ya existen items/widgets)
        st = (r.get("estado", "") or "").upper()
        if "PENDIENTE" in st:
            self._paint_row_bg(i, PENDIENTE_BG)
        elif "EN PROCESO" in st:
            self._paint_row_bg(i, EN_PROCESO_BG)
        elif "FINAL" in st:
            self._paint_row_bg(i, FINALIZADA_BG)
        elif "CANCEL" in st:
            self._paint_row_bg(i, CANCELADA_BG)
    ...
    def _export_excel(self):
        # 1) Datos visibles
        rows = getattr(self, "_rows_cur", None) or []
        if not rows:
        ...
        try:
            ...
            QMessageBox.warning(self, "Falta dependencia",
                                "Instala openpyxl:\n\npip install openpyxl")
            return

        # 4) Definición de columnas (orden y llaves de rows)
        cols = [
            ("ID", "id_orden"),
            ("Estado", "estado"),
            ("Fecha", "FECHA"),
            ...
            ("Consec", "CONSECUTIVO"),
            ("Material", "material"),
            ("Ancho roll", "ancho_rollo_cm"),
            ("Largo", "LARGO"),
            ("Rep", "REP"),
            ("Impreso(cm)", "impreso_cm"),
            ("Consumo esperado(cm)","consumo_esp_cm"),
            ("Desp.L", "desp_largo_cm"),
            ("Desp.A(cm)", "desp_ancho_cm"),
            ("Lam.tipo", "lam_tipo"),
            ("Lam.marca", "lam_marca"),
            ("Lam.ancho", "lam_ancho_cm"),
            ("Lam.consumo", "lam_consumo_cm"),
            ("Merma cancel(cm)", "merma_cancel_cm"),
            ("Ruta", "ruta"),
        ]
        ncols = len(cols)
        ...
        # 6) Encabezado con estilo
        head_fill = PatternFill("solid", fgColor="00897B")     # verde azulado
        ...
        for r_idx, r in enumerate(rows, start=2):
            for c_idx, (_, key) in enumerate(cols, start=1):
                val = r.get(key, "")
                ws.cell(row=r_idx, column=c_idx, value=val).border = border

        # 8) Formatos: fechas, números y anchos
        # Índices de columnas (1-based) según 'cols'
        col_date = [3, 4]  # Fecha, Entrega
        col_num = [7, 8, 9, 10, 11, 12, 15, 16, 17]    # ancho roll, largo, rep, impreso, desp.l, desp.a, lam.ancho, lam.consumo, merma
        ...
        widths = [8, 12, 12, 12, 14, 28, 11, 11, 7, 12, 10, 10, 14, 14, 11, 12, 16, 40]
        for i, w in enumerate(widths, start=1):
            ws.column_dimensions[get_column_letter(i)].width = w

        # 9) Zebra striping y colores por estado
        ...
        for r in range(2, len(rows) + 2):
            ...
            # Colorear estado (columna 2)
            estado = (ws.cell(row=r, column=2).value or "").upper()
            if "FINAL" in estado:
                ws.cell(row=r, column=2).fill = PatternFill("solid", fgColor="C6EFCE")  # verde claro
            elif "CANCEL" in estado:
                ws.cell(row=r, column=2).fill = PatternFill("solid", fgColor="FFC7CE")  # rojo claro
            elif "PROCESO" in estado:
                ws.cell(row=r, column=2).fill = PatternFill("solid", fgColor="FFF2CC")  # amarillo claro

        # 10) Congelar encabezado + autofiltro + tabla con estilo
        ws.freeze_panes = "A2"
        last_cell = f"{get_column_letter(ncols)}{len(rows) + 1}"
        tbl = Table(displayName="ReporteGF", ref=f"A1:{last_cell}")
...
class PagImpresion(QWidget):
    ...
    "ID","Estado","Fecha","Entrega","Material","ancho roll","Largo","Rep",
        ...
        "Merma cancel(cm)","Esp.reps(cm)","Observaciones","Ruta","Acciones"
    ] 
    ...
    def _build(self):
        ...
        self.fil_ent_desde = QDateEdit(); self.fil_ent_desde.setCalendarPopup(True); self.fil_ent_desde.setDisplayFormat("yyyy-MM-dd")
        self.fil_ent_hasta = QDateEdit(); self.fil_ent_hasta.setCalendarPopup(True); self.fil_ent_hasta.setDisplayFormat("yyyy-MM-dd")
        self.fil_material  = QLineEdit(); self.fil_material.setPlaceholderText("Material contiene…")
        self.fil_ancho_min = QDoubleSpinBox(); self.fil_ancho_min.setDecimals(2); self.fil_ancho_min.setMaximum(10_000_000)
        self.fil_ancho_max = QDoubleSpinBox(); self.fil_ancho_max.setDecimals(2); self.fil_ancho_max.setMaximum(10_000_000)
        ...
        self.fil_material.textChanged.connect(self._apply_filters)
        self.fil_ancho_min.valueChanged.connect(self._apply_filters)
        self.fil_ancho_max.valueChanged.connect(self._apply_filters)

        flt.addWidget(QLabel("Entrega desde:")); flt.addWidget(self.fil_ent_desde)
        flt.addWidget(QLabel("hasta:"));        flt.addWidget(self.fil_ent_hasta)
        flt.addSpacing(12)
        flt.addWidget(QLabel("Material:"));     flt.addWidget(self.fil_material, 1)
        flt.addSpacing(12)
        flt.addWidget(QLabel("Ancho roll ≥"));  flt.addWidget(self.fil_ancho_min)
        flt.addWidget(QLabel("≤"));             flt.addWidget(self.fil_ancho_max)
        flt.addStretch(1)
        ...
        widths = [60,110,110,110,200,90,90,60,110,110,210,110,110,115,125,110,220,320,220]
        for i, w in enumerate(widths):
            self.tbl.setColumnWidth(i, w)
    ...
    def _filtrar_finalizadas_ultimas_2_semanas(self, rows):
        """Si la casilla está activa, deja sólo FINALIZADAS con FECHA >= hoy-14d."""
        if not self.chk_incluir_fin.isChecked():
            return rows
        limite = QDate.currentDate().addDays(-14).toString("yyyy-MM-dd")
        out = []
        for r in rows:
    ...
    def _reset_filters(self):
        # Rango de fechas “abierto” por defecto
        self.fil_ent_desde.setDate(QDate(1900, 1, 1))
        self.fil_ent_hasta.setDate(QDate(2999, 12, 31))
        self.fil_material.setText("")
        # 0 = sin límite; si pones sólo min o sólo max también funciona
        self.fil_ancho_min.setValue(0.0)
        self.fil_ancho_max.setValue(0.0)
        self._apply_filters()
    ...
    def _apply_filters(self):
        """Oculta/muestra filas combinando: Entrega (rango), Material (contiene), Ancho roll (rango)."""
        d0 = self.fil_ent_desde.date()
        d1 = self.fil_ent_hasta.date()
        needle = self.fil_material.text().strip().lower()
        fmin = float(self.fil_ancho_min.value() or 0.0)
        fmax = float(self.fil_ancho_max.value() or 0.0)
        ...
        for r in range(self.tbl.rowCount()):
            ...
            # Material contiene -> columna 4
            if ok and needle:
                mat = self._cell_str(r, 4).lower()
                if needle not in mat:
                    ok = False

            # Ancho roll -> columna 5
            if ok and (fmin > 0.0 or fmax > 0.0):
                aval = self._cell_float(r, 5)
                if aval is None:
                    ok = False
                else:
                    if fmin > 0.0 and aval < fmin:
                        ok = False
                    if fmax > 0.0 and aval > fmax:
                        ok = False
    ...
    def _add_row(self, r: dict):
        ...
        # ----- columnas 0..7 (texto plano) -----
        set_text(0, r.get("id_orden", ""))
        set_text(1, r.get("estado", ""))
        set_text(2, r.get("FECHA", ""))

        # Entrega (col 3): la seteamos normal y luego la estilamos
        fecha_entrega_str = str(r.get("fecha_entrega", "") or "")
        set_text(3, fecha_entrega_str)

        set_text(4, r.get("material", ""))
        ...
        if lam_marca_id is not None:
            for idx in range(cb_marca.count()):
                if cb_marca.itemData(idx) == lam_marca_id:
                    cb_marca.setCurrentIndex(idx); break
        ...
        self.tbl.setCellWidget(i, 18, cell)
    ...
    def _validate_finalize(self, row) -> bool:
        ...
        lam_ancho = self._g(row, 12, float) or 0.0
        lam_cons  = self._g(row, 13, float)
        lam_marca = self._g(row, 11, int)
        ...
        if lam_tipo_txt != "SIN LAMINAR":
            ...
            try:
                ancho_roll = float(self.tbl.item(row, 5).text())
            except Exception:
                ancho_roll = None
    ...
    def _row_finalizar(self, row, oid):
        ...
        payload = {
            "id_orden": oid,
            "total_largo_impreso_cm": self._g(row, 6, float),
            "desp_largo_cm":           self._g(row, 8, float),
            "desp_ancho_cm":           self._g(row, 9, float),
            "repeticiones":            self._g(row, 7, int),
            "usuario_impresor": None,
            "estado_requerido": "EN PROCESO",
            "ancho_objetivo_cm":       self._g(row, 12, float),
            "lam_tipo_id":             tipo_id,                       # << aquí
            "lam_marca_id":            self._g(row, 11, int),         # puede ser None
            "lam_ancho_cm":            self._g(row, 12, float),
            "lam_consumo_cm":          self._g(row, 13, float)
        }
...
class Main(QMainWindow):
    ...
    def __init__(self):
        ...
        usuario = auth.get("usuario") or auth.get("user") or auth.get("email") or ""
        if not token:
        ...
        self.statusBar().showMessage(f"Sesión: {usuario or 'usuario'} · Rol: {rol_raw or '—'}", 5000)
        ...
        self._collapsed_w = 44          # ancho visible cuando está plegado
        self._expanded_w  =  max(180,  self.width() // 8)  # recordable, ver más abajo
        ...
        # céntralo horizontalmente para que no “se pegue” al borde
        left_v.setAlignment(self.btn_logo, Qt.AlignHCenter)
        left_v.addWidget(self.btn_logo, 0, Qt.AlignLeft)
        ...
        self.splitter.setCollapsible(0, True)   # permitir colapsar el panel izquierdo
        self.splitter.setStretchFactor(0, 0)    # el izquierdo NO se estira
        self.splitter.setStretchFactor(1, 1)    # el derecho SÍ se estira
        
        self._sidebar_collapsed = False
        self._sidebar_last_w = 180

        # tamaño inicial razonable
        self.splitter.setSizes([self._sidebar_last_w, max(600, self.width()-self._sidebar_last_w)])

        # este panel debe poder encogerse, pero no a 0 cuando colapsa
        left_panel.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Expanding)


        # Tamaños iniciales
        self._sidebar_last_w = 180               # ancho recordado del menú
        self._sidebar_collapsed = False
        self.splitter.setSizes([self._sidebar_last_w, 1100])

        # Guardar el ancho del sidebar si el usuario mueve el divisor
        self.splitter.splitterMoved.connect(self._on_splitter_moved)

        self.setCentralWidget(self.splitter)
        self._left_panel = left_panel           # referencia para límites
        try:
    ...
    def _on_splitter_moved(self, pos: int, index: int):
        if not self._sidebar_collapsed:
            ...
            if w >= 120:                # evita guardar anchos muy chicos
    ...
    def _toggle_sidebar(self):
        ...
        if self._sidebar_collapsed:
            ...
            # Márgenes normales cuando está expandido
            lp_layout: QVBoxLayout = self._left_panel.layout()
            if lp_layout:
